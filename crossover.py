...
class CrossoverForPixels:
    ...
    def single_point(self, parent_one: np.ndarray, parent_two: np.ndarray):
    ...
    def two_point(self, parent_one: np.ndarray, parent_two: np.ndarray):
        size = min(parent_one.size, parent_two.size)
    ...
    def uniform(self, parent_one: np.ndarray, parent_two: np.ndarray):
        size = min(parent_one.size, parent_two.size)
    ...
    def perform_crossover(self, mating_pool: np.ndarray, method: str="single_point", number_of_offsprings: int=8) -> np.ndarray:
        offsprings = []
